...
st.caption("Note: Raw model accuracy is ~60%. Adjusted scores are shown for demo clarity.")
...
if st.button("Predict"):
    if not description.strip():
        ...
        with st.spinner("Predicting..."):
            ...
            raw_score = reg.predict(X_input)[0]

            # ====== Adjusted score for demo ======
            adjusted_score = raw_score
            if predicted_class == "Easy":
                adjusted_score = np.clip(adjusted_score, 800, 1200)
            elif predicted_class == "Medium":
                adjusted_score = np.clip(adjusted_score, 1400, 1800)
            elif predicted_class == "Hard":
                adjusted_score = np.clip(adjusted_score, 2000, 3000)

            # Short text correction
            if text_len < 50:
                adjusted_score = max(adjusted_score, 1200)
        ...
        st.info(f"Raw Model Score: {raw_score}")
        st.info(f"Displayed Difficulty Score (for demo): {adjusted_score}")
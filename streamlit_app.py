...
if st.button("Predict"):
    if not description.strip():
        ...
        with st.spinner("Predicting..."):
            X_input, text_len = extract_features(title, description, input_desc, output_desc)
            predicted_class = clf.predict(X_input)[0]
            predicted_score = reg.predict(X_input)[0]

            # ====== Adjust predictions based on text length and class ======
            if predicted_class == "Medium" and predicted_score < 1200:
                predicted_score = 1400  # raise floor for Medium
            elif predicted_class == "Hard" and predicted_score < 1800:
                predicted_score = max(1800, predicted_score)
            elif predicted_class == "Easy" and predicted_score < 800:
                predicted_score = 800

            # Short problems correction
            if text_len < 50:
                predicted_score = max(predicted_score, 1000)  # prevent too low score for short text

            predicted_score = int(round(np.clip(predicted_score, 800, 3500)))
